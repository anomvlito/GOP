...
\end{itemize}
    \begin{quotation}
    \small \textit{\textbf{Nota intuitiva:} ¿De dónde sale esta función? La imprenta busca minimizar la suma de sus dos ``fugas de dinero'' por hora: (1) La plata que deja de ganar por los descuentos ($\lambda \times \Delta$), y (2) El costo o castigo social de tener a los clientes esperando en la fila ($\lambda \times CE(W)$). ¡Por eso simplemente se suman!}
    \end{quotation}
...
\end{itemize}
    \begin{quotation}
    \small \textit{\textbf{Nota matemática:} La expresión de abajo sale de aplicar la \textbf{Regla del Producto} a la función objetivo $f(\Delta) = u \cdot v$, donde $u = \lambda(\Delta)$ y $v = [\Delta + 100 + W(\Delta)]$. Su derivada es $u'v + uv'$. Recuerda que al derivar $u = \lambda_0 e^{-\Delta}$, baja un signo menos, quedando $u' = -\lambda_0 e^{-\Delta} = -\lambda(\Delta)$.}
    \end{quotation}
    Derivando la función objetivo $f(\Delta)$ con respecto a $\Delta$ e igualando a cero:
    $$\frac{df}{d\Delta} = 0 \implies \underbrace{-\lambda(\Delta)}_{u'} \underbrace{\left[ \Delta + 100 + W(\Delta) \right]}_{v} + \underbrace{\lambda(\Delta)}_{u} \underbrace{\left[ 1 + \frac{dW}{d\Delta} \right]}_{v'} = 0$$
